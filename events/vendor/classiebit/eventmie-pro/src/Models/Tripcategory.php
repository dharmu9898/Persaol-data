...
{
    return Trip::leftJoin("categories", "categories.id", '=', "trips.category_id")
            ->select(["trips.*", "categories.name as category_name"])
            ->where(['trips.featured' => 1, 'events.publish' => 1, 'events.status' => 1, 'categories.status' => 1])
