...
use Classiebit\Eventmie\Mail\Confirm_Booking_Event;
use Classiebit\Eventmie\Mail\Confirm_Event_Booking;
use Classiebit\Eventmie\Mail\Event_Booking_Confirm;
...
class BookingsController extends Controller
{
    ...
    {
        Log::info('is_admin_organiser9');
        Log::info($request);

        $currece=Ticket::where('event_id',$request->event_id)->first();
        Log::info('ticket ka data');
        Log::info($currece);
        Log::info('Currency name ka data');
        $currency_name= $currece->currency_name;
        $curre_name=substr($currency_name,0, 3);
        Log::info($curre_name);



        $detail=Event::where('event_id',$request->event_id)->first();
        Log::info('Event ka data');
        Log::info($detail);
        $opmail = $detail->operator_email;
        $name = $detail->operator_name;
        $eventtitle = $detail->title;
        $address = $detail->country_state_city;
        $stdate = $detail->start_date;
        $enddate = $detail->end_date;
        $pincode = $detail->zipcode;
        Log::info($opmail);

        Log::info('st and end date');
        ...
        $userdetail=User::where('email',$emaillogin)->first();
        Log::info('userdetail');
        Log::info($userdetail);
        ...
        $operdetail=User::where('email',$opmail)->first();
        Log::info('operdetail');
        Log::info($operdetail);
        $opphone = $operdetail->phone;
        ...
        $roledetail=Event::where('event_id',$request->event_id)->first();

        Log::info('Event ka data organiser_id');
        Log::info($roledetail);
        $organiser_id = $roledetail->user_id;
        Log::info('user id aa raha hai');
        Log::info($organiser_id);


        $emaillogin=Auth::User()->email;

        $customerdetails=User::where('email',$emaillogin)->first();
        Log::info('customerdetails');
        Log::info($customerdetails);

        $usermail = $customerdetails->email;
        ...
        foreach($selected_tickets as $key => $value)
        {
            ...
            $booking[$key]['customer_id']       = $userid;
            $booking[$key]['customer_name']     = $username;
            $booking[$key]['customer_email']    = $usermail;
          
            $booking[$key]['organiser_id']       = $organiser_id;
            ...
            Log::info('repetitive date aa raha hai start');
            Log::info( $booking_date);

            Log::info('repetitive date aa raha hai End');
            Log::info(  $booking[$key]['event_end_date']);
            ...
            Log::info('quantity aa raha hai');
            Log::info( $params['quantity']);
            // calculating net price
            $net_price    = $this->calculate_price($params);
            $person=  $params['quantity'];
            Log::info('quantity aa raha hai 8='. $person);
        ...
        $is_direct_checkout = $this->checkDirectCheckout($request, $total_price);
          ...
          //    Send mail Admin and oraginal, user 


          if($booking_date==$booking[$key]['event_end_date']){
            $endbooking=  $booking[$key]['event_end_date'];
            Log::info('endbooking aa raha hai ='. $endbooking);
            $stabooking=  $booking_date;
            Log::info('stabooking aa raha hai ='. $stabooking);

          $eveadminMailsend = array(  
            'email' => $admin->email,

         
            'opname' => $name, 
            'opemail' => $opmail,    
            'Cuntry_state_city' => $address,    
            'title' => $eventtitle,    
            'start_date' => $stabooking,    
            'end_date' => $endbooking,    
            'Zim_code' => $pincode, 
            ...
            Mail::to($eveadminMailsend['email'])->send(new Confirm_Event_Booking($eveadminMailsend));
                log::info('after mail if');


               

        $EvebtopMailsend = array(  
            'opevemail' => $opmail,
            ...
            'title' => $eventtitle, 
            'per_cost' => $price,
            'quantity_person' => $person,
            'total_cost' => $total_price, 
            'currency_name' => $curre_name,  
     );
     Mail::to($EvebtopMailsend['opevemail'])->send(new Confirm_Booking_Event($EvebtopMailsend));
     log::info('after mail cokies this');

     $UsereventMailsend = array(  
        'usermail' => $emaillogin, 
 ...
 Mail::to($UsereventMailsend['usermail'])->send(new Event_Booking_Confirm($UsereventMailsend));
 log::info('after mail cokies this');
}
...
log::info('after mail cokies this');
    ...
    */
    ...
    protected function finish_booking($booking = [], $data = [])
    {
        ...
        foreach($booking as $key => $value)
        {
            log::info('make_booking aa raha hai');
            $data     = $this->booking->make_booking($params[$key]);
            log::info('make_booking ke baad aa raha hai');
            $booking_data[] = $data;
